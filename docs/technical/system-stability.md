...
| 2026-04-01 post-#153 (planned) | 0 | Manual rescue: both sessions moved to `~/.config/mem-watchdog/chat-archives/frankspressurewashing-20260401-125251/`. FPW workspace now loads cleanly. Guard scope fix to scan all workspaceStorage directories in background (not just the currently open workspace) is tracked in issue #153. |
...
| Chat Continuity Guard — cross-workspace scope | Pathway #1 — guard only scans current workspace; oversized sessions in other workspaces go undetected until that workspace is opened | ❌ Gap (issue #153) |
