...
| 2026-03-31 15:03–15:16 | 1 (freeze, non-OOM) | Stage 4 correctly SIGKILLed Chrome/Playwright at 15% free memory, but Stage 3 reclaim then called `memory.force_empty` twice in close succession. The second synchronous write (`echo 0 | sudo -n tee .../memory.force_empty`) blocked for 13m15s, stalling the watchdog loop and creating an intervention blind spot during an active automation window. |
| 2026-03-31 post-fix | 0 | Daemon v20260331.2: cgroup reclaim writes are bounded with `timeout`, and reclaim attempts are rate-limited via `RECLAIM_MIN_INTERVAL_S`. Stage 3 logs now distinguish reclaim-applied vs reclaim-skipped outcomes for accurate incident forensics. |
| 2026-04-01 12:45 | 1 (OOM) | Cross-workspace session-load OOM: user opened the frankspressurewashing workspace, which had two oversized Copilot sessions (458 MB + 357 MB = 815 MB) in active chatSessions storage. VS Code loaded both at startup, spiking RSS from 1.3 GB to 4.4 GB in ~38 seconds. Daemon reached Stage 4 at pct=11%, found no disposable target (no Chrome, no automation), deferred to kernel OOM. Kernel OOM-killed a process (oom_kill 12→13). Second Stage 4 at pct=8% → second kernel OOM kill. Root cause: Chat Continuity Guard only scans the *current* workspace at the time the extension activates — it did not see the FPW sessions growing while the crostini-mem-watchdog workspace was active. By the time the FPW window opened, both sessions were already oversized and were loaded before the 60s guard timer could fire. (issue #153) |
| 2026-04-01 post-#153 | 0 | Extension guard now runs a preemptive startup scan and archives oversized inactive-workspace sessions before workspace switch load. Daemon adds startup `CHAT_WARN_MB` scan of `workspaceStorage/*/chatSessions/` and warns when any workspace exceeds threshold. Manual rescue archive retained at `~/.config/mem-watchdog/chat-archives/frankspressurewashing-20260401-125251/`. |
| 2026-04-03 post-#157 | 0 | Daemon/extension interactive kill-approval handshake added for non-critical disposable interventions: daemon writes request file, extension returns allow/defer response, daemon enforces TTL and bounded defer cooldown. Loop remains non-blocking and Stage 4/EMERGENCY paths remain ungated. |
| 2026-04-07 post-#163/#164 | 0 | **Process Kill Safety (v0.3.22 / v20260406.1)**: (1) Shared Process (`node.mojom.NodeService` without `--inspect-port`) now excluded from `kill_top_vscode_helper` fallback/last-resort at non-critical mode — VS Code Shared Process has zero auto-restart, killing it caused "Please restart" toast requiring full relaunch; (2) `_classify_helper_type()` and awk `classify()` return `"extension-host"` for `--inspect-port` processes — Extension Host was previously classified as `"other"`, allowing repeated re-kills that cascaded into 5× HTML Language Server deaths; (3) Startup pre-emptive kill now reads `/proc/meminfo` inline + gates on `STARTUP_PREEMPT_PCT_THRESHOLD` (40%) — previously killed Playwright sessions at 76%+ free memory on every workspace switch; (4) `humanizeReason()` startup branch added in extension.js. |
...
| Bounded cgroup reclaim latency (v20260331.2) | Pathway #1 — `memory.reclaim` / `memory.force_empty` writes are now timeout-bounded and spaced by a minimum reclaim interval. Prevents long synchronous reclaim calls from blocking the watchdog loop and creating intervention blind spots during high pressure. | ✅ Fixed |
| Container swap | Pathway #1 — direct relief for container kernel OOM | ❌ Blocked (CapEff=0, see §4) |
| Chat Continuity Guard — cross-workspace scope fix (issue #153) | Pathway #1 — guard now preemptively archives oversized inactive-workspace sessions on startup/timer; daemon startup scan warns when workspace chat footprint already exceeds `CHAT_WARN_MB` | ✅ Fixed |
| Interactive non-critical kill approval (issue #157) | Pathway #1 — non-critical disposable kills now support operator allow/defer via file handshake with TTL + cooldown, reducing avoidable automation interruptions while preserving ungated emergency safety actions. | ✅ Fixed |
| Shared Process protection (v20260406.1) | Pathway #1 — `kill_top_vscode_helper` fallback/last-resort excludes `node.mojom.NodeService` utility processes without `--inspect-port` at non-critical mode. Shared Process has zero auto-restart; killing it required full VS Code relaunch. `#80` comment claiming auto-restart was incorrect for Shared Process. | ✅ Fixed |
| Extension Host anti-respawn classification (v20260406.1) | Pathway #1 — `_classify_helper_type()` and awk `classify()` now return `"extension-host"` for `--inspect-port` processes. Prevents repeated Extension Host kills cascading into 5× HTML Language Server deaths (vscode-languageclient permanent death threshold). | ✅ Fixed |
| Startup pre-emptive kill memory gate (v20260406.1) | Pathway #1 — startup pre-emptive Chrome SIGTERM reads `/proc/meminfo` inline and fires only when `MemAvailable ≤ STARTUP_PREEMPT_PCT_THRESHOLD` (default 40%). Prevents killing active Playwright sessions at 76%+ free memory on workspace switch. | ✅ Fixed |
